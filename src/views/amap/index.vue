  ...
  methods: {
    ...
    showActSimDominoRadiusLayers(point, data) {
      ...
      if (data.deadRadius) {
        ...
        let f = this.setCircleFeature(point, data.deadRadius, color);
        features = f;
      }
    ...
    setCircleFeature(point, radius, color) {
      // let geom = new ol.geom.Circle(
      //   [point.longitude, point.latitude],
      //   radius / 100000
      // );
      let geomPoint = new Point([point.longitude, point.latitude]);
      let f = new Feature({
        geometry: geomPoint,
        name: "测试点位",
      });
